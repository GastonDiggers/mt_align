...
# Function to save the TMX (Translation Memory eXchange) file
def save_tmx_file(file_path, output_path, tmx_obj, target_language, source_language):
    ...
    output_file_path = os.path.join(
        output_path, f"{os.path.splitext(os.path.basename(file_path))[0]}-{target_language}-{source_language}.tmx")
    ET.ElementTree(tmx_obj).write(output_file_path,
                                  encoding="utf-8", xml_declaration=True)

    # Open the output file and insert a newline character after each closing </tu> tag for readability
...
def translate_xml(file_path, source_language, target_language, output_path):
    ...
    # Save the translated TMX file
    save_tmx_file(file_path=file_path, output_path=output_path, tmx_obj=tmx_obj,
...
def translate_files(input_folder, source_language, target_language):
    ...
    data_files = [os.path.join(input_folder, f)
                  for f in os.listdir(input_folder)]

    # Create the output folder if it doesn't exist
    output_path = os.path.join(
        input_folder, f"output_{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}")
